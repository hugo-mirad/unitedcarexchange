...
function GetVehicleType() {
    CarsService.GetVehicleType(OnSuccessGetVehicleType, onError);
}

function OnSuccessGetVehicleType(result, userContext, methodName) {
    try {

        //alert(result);
        //make = result;
        var json = xmlToJson(result);
        if (json["ArrayOfVehicleTypeInfo"]["VehicleTypeInfo"] != undefined) {
            if (json["ArrayOfVehicleTypeInfo"]["VehicleTypeInfo"].length != undefined) {
                BindVehicleType(json["ArrayOfVehicleTypeInfo"]["VehicleTypeInfo"]);
...
function GetCategory() {
    CarsService.GetCategory(OnSuccessGetCategory, onError);
}
function OnSuccessGetCategory(result, userContext, methodName) {
    try {
        ...
        /*
        if (json["ArrayOfCategoryInfo"]["CategoryInfo"] != undefined) {
            if (json["ArrayOfCategoryInfo"]["CategoryInfo"].length != undefined) {
                console.log(json["ArrayOfCategoryInfo"]["CategoryInfo"]);
                BindCategory(json["ArrayOfCategoryInfo"]["CategoryInfo"]);
...
function OnSuccessGetMakes(result, userContext, methodName) {
    try {
       ...
       //alert(result);
        ...
        // alert(result);
        if (json["ArrayOfMakesInfo"]["MakesInfo"] != undefined) {
            if (json["ArrayOfMakesInfo"]["MakesInfo"].length != undefined) {
                BindMakes(json["ArrayOfMakesInfo"]["MakesInfo"]);
...
function CarsSearch(carMake, CarModal, ZipCode, WithinZip,VehicleType, Category ) {

    window.location.href = 'SearchTrucks.aspx?Make=' + carMake + '&Model=Any Model' + '&ZipCode=' + ZipCode + '&WithinZip=' + WithinZip + '&VehicleType=' + VehicleType + '&Category=' + Category + '';
}



function CarsSearch2(carMakeid, CarModalId, ZipCode, WithinZip, pageNo, pageresultscount, orderby, VehicleType, Category) {
    //console.log(carMakeid, 'Any Model', ZipCode, WithinZip, pageNo, pageresultscount, orderby, VehicleType, Category);
    CarsService.GetCarsSearch(carMakeid, 'Any Model', ZipCode, WithinZip, pageNo, pageresultscount, orderby,VehicleType, Category, OnSuccessCarsSearch, onError);
}
...
function CarsSearchPage(carMakeid, CarModalId, ZipCode, WithinZip, pageNo, pageresultscount, orderby, VehicleType, Category) {

    CarsService.GetCarsSearch(carMakeid, CarModalId, ZipCode, WithinZip, pageNo, pageresultscount, orderby, VehicleType, Category, OnSuccessCarsSearchPage, onError);
...
function EncryptedCarID(sCarID, sMake, sModel, sZip, sWithinZip, sVehicleType) {


    var v = 'SearchTrucksDetails.aspx?VehicleType=' + sVehicleType + '&Make=' + sMake + '&ZipCode=' + sZip + '&WithinZip=' + sWithinZip + '&C=' + randomString() + '' + sCarID + '';
...
function OnSuccessGetZips(result, userContext, methodName) {
    try {
        ...
        if (result == true) {
           //console.log($('#make option:selected').text(), $('#model').val(), $('#yourZip').val(), $('#within option:selected').val(), $('#vehicleType option:selected').text(), $('#categoty option:selected').val())
            CarsSearch($('#make option:selected').text(), $('#model option:selected').val(), $('#yourZip').val(), $('#within option:selected').val(), $('#vehicleType option:selected').text(), $('#categoty option:selected').text());
            //alert($('#make option:selected').text()+", "+ $('#model').val()+", "+ $('#yourZip').val()+", "+ $('#within option:selected').val()+", "+ '1'+", "+ '25'+", "+ 'price'+", "+ $('#vehicleType option:selected').val());
            CarsService.SearchCriteriaSave($('#make option:selected').text(), $('#model option:selected').val(), $('#yourZip').val(), $('#within option:selected').val(), $('#vehicleType option:selected').text(), $('#categoty option:selected').text(), OnSuccessSearchCriteriaSave, onerror);
        } else {
            hideSpinner();
...
function OnSuccessGetZips2(result, userContext, methodName) {
    try {
        ...
        if (result == true) {
            ...
            //CarsSearch('All makes', zip1, '5', '1', '25', 'price', VehicleID1);
            //CarsService.SearchCriteriaSave('All makes','', '0', zip1, '5', OnSuccessSearchCriteriaSave, onerror);

            CarsSearch('Any Make', 'Any Model', zip1, '3', VehicleIDName, Category);
            CarsService.SearchCriteriaSave('Any Make', 'Any Model', zip1, '3', VehicleIDName, Category, OnSuccessSearchCriteriaSave, onerror);
...
function GetCarPictures(sCarId) {
    try {
        CarsService.GetCarPictures(sCarId, OnSuccessGetCarPictures, onError);
    } catch (ex) { }

}

function OnSuccessGetCarPictures(result, userContext, methodName) {
    try {
        var json = xmlToJson(result);
        //alert(json);         
        var Details = json["ArrayOfCarPicturesInfo"]["CarPicturesInfo"];
        vehiclePicData(Details);
        //carFretures(json);
    } catch (ex) { }
