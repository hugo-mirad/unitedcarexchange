    ...
    if ((SearchResultsArray != undefined && SearchResultsArray != null) || (SearchResultsArray['Make'] != null && SearchResultsArray['Make'] != '')) {
        ...
        if (SearchResultsArray != null && SearchResultsArray != '' && SearchResultsArray.length > 0) {
            ...
            for (i = 0; i < SearchResultsArray.length; i++) {
                ...
                m += "</p><a href='javascript:void(0);' class='viewA'>View seller details</a><p id='" + SearchResultsArray[i]['Carid']['#text'] + "a' class='details'><strong>Seller Type: </strong> " + SearchResultsArray[i]['SellerType']['#text'] + "<br><strong>Seller Details: </strong>"
                ...
                if (SearchResultsArray[i]['City']['#text'] != 'Emp' || SearchResultsArray[i]['State']['#text'] != 'Emp' && SearchResultsArray[i]['State']['#text'] != 'UN') {
                    m += ")</span><br /><span class='userNumber'><strong>Phone: </strong>";
                } else {
                ...
                if (SearchResultsArray[i]['IsActive']['#text'] == 'false') {
                    ...
                    $("p." + SearchResultsArray[i]['Carid']['#text'] + "a").empty().append(sorry);
                }
            }  // For loop close		
...
function oneReord(SearchResultsArray) {
    ...
    var withinZIp1 = $('#within option:selected').val();
    var zip1 = $('#yourZip').val();
    var resPath = '"' + SearchResultsArray['Carid']['#text'] + '","' + SearchResultsArray['Make']['#text'] + '","' + SearchResultsArray['Model']['#text'] + '","' + zip1 + '","' + withinZIp1 + '"';
    var thumb1 = '';
...
function carDetailsDisplay() {
    ...
    try {
        if (carDetails != undefined) {
            var m = '';
            selectedCarID = carDetails['Carid']['#text'];
            ...
            var meta = "<meta name='Description' content='" + carDetails['Description']['#text'] + "'/>";
            var meta = "<meta name='keywords' content='" + carMakeModel + " Used Cars, New Cars, Used Cars Online, Used Cars for Sale, New Cars for Sale, Secondhand Cars Online, Find Cars Online, Used Vehicles online, Search Used Cars, Search New Cars'/>";
            ...
            if (slideImg.length > 0) {
                ...
                if (imgCount > 19) {
                    ...
                    */
                    ...
                    //$('div#basic2 ul li img').css({'width':'680px', 'height':'auto'});
        ...
        //alert($('#model option').attr('selected').val());

        //$('#make option[value='+3+']').attr('selected', 'selected');
        /* 
        ...
        $('#model option[value='+3+']').attr('selected', 'selected');           
...
function carFretures(result) {
    ...
    //p+= '<strong style="float:left">States: </strong><span class="States" style="font-size:11px;"></span>'

    // Similar Cars:-->   p+= '<br><div class="clear">&nbsp;</div><strong>Similar to the class of cars such as: </strong><span span class="similarCars" style="font-size:11px;"></span>'
...
function bindNearCities(result) {
    ////console.log(result);      

    for (i = 0; i < result.length; i++) {
        var city = result[i].split(',')
        //$('span.towns').append(city[2]);
        $('span.zip').append(city[1]);
        if (i < result.length - 1) {
            //$('span.towns').append(', &nbsp;');
            $('span.zip').append(', &nbsp;');
        }
    }
    //$('span.towns').append('<div style="clear:both; height:5px; overflow:hidden">&nbsp;</div>');
    $('span.zip').append('<div style="clear:both; height:5px; overflow:hidden">&nbsp;</div>');
}
function bindNearStates(result) {
    ////console.log(result);    
    for (i = 0; i < result.length; i++) {
    ...
    $('ul.towns').append('<div style="clear:both; height:5px; overflow:hidden">&nbsp;</div>');
}


function bindSimilarCars(result) {
    ////console.log(result);
    if (result.length > 0) {
        for (i = 0; i < result.length; i++) {
            if (selectedCarID != result[i]['Carid']['#text']) {
                var m = '';
                m += result[i]['YearOfMake']['#text'] + ' ' + result[i]['Make']['#text'] + ' ' + result[i]['Model']['#text'];
                /*if(result[i]['Price']['#text'] != 'Emp' && result[i]['Price']['#text'] != '0' && result[i]['Price']['#text'] != null && result[i]['Price']['#text'] != 'Unspecified'){
                m += ' - <span class="detPrice20">'+result[i]['Price']['#text']+'</span>'
                }*/
                $('span.similarCars').append(m);
                if (i < result.length - 1) {
                    $('span.similarCars').append(', &nbsp; ');
                }
            }
        }
        $('.detPrice, .detPrice20').formatCurrency();
        $('span.similarCars').append('<div style="clear:both; height:5px; overflow:hidden">&nbsp;</div>');
    }
}
