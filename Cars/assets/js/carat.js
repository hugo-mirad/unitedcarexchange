...
$(this).parent().append(' <div class="imageLoader" style="width:' + $(this).width() + 'px; height:' + $(this).height() + 'px" ></div>');
$(this).hide();
$(this).load(function() {
$(this).parent().children('div.imageLoader').remove();
$(this).fadeIn();
}).error(function() {
$(this).parent().children('div.imageLoader').remove();
$(this).fadeIn();
...
function _initGallery() {
    ...
    var length1 = $('.mainSlides img').length;
    //console.log(length1)
    if (length1 > 0) {
        var img = '';
        for (i = 0; i < length1; i++) {
            var path = $('.mainSlides img:eq(' + i + ')').attr('src');
            img += '<div class="thumbnail-' + i + '">';
            img += "<img src='" + path + "' alt='#'>";
            img += '</div> ';
        }
        //console.log(img);
        $('.gallery-thumbnails').html(img);
