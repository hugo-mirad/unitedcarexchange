...
function CarsSearch(carMake, CarModal, ZipCode, WithinZip,VehicleType ) {
    window.location.href = 'SearchCars.aspx?Make=' + carMake + '&Model=' + CarModal + '&ZipCode=' + ZipCode + '&WithinZip=' + WithinZip + '&VehicleType='+ VehicleType +'';
}



function CarsSearch2(carMakeid, CarModalId, ZipCode, WithinZip, pageNo, pageresultscount, orderby, VehicleType) {
    CarsService.GetCarsSearch(carMakeid, CarModalId, ZipCode, WithinZip, pageNo, pageresultscount, orderby,VehicleType, OnSuccessCarsSearch, onError);
}
...
function CarsSearchPage(carMakeid, CarModalId, ZipCode, WithinZip, pageNo, pageresultscount, orderby, VehicleType) {

    CarsService.GetCarsSearch(carMakeid, CarModalId, ZipCode, WithinZip, pageNo, pageresultscount, orderby, VehicleType, OnSuccessCarsSearchPage, onError);
...
function EncryptedCarID(sCarID, sMake, sModel, sZip, sWithinZip, sVehicleType) {
    ...
    var v = 'SearchCarDetails.aspx?Make=' + sVehicleType + '&Model=' + sMake + '&ZipCode=' + sZip + '&WithinZip=' + sWithinZip + '&C=' + randomString() + '' + sCarID + '';
...
function OnSuccessGetZips(result, userContext, methodName) {
    try {
        ...
        if (result == true) {
            CarsSearch($('#make option:selected').text(), $('#model').val(), $('#yourZip').val(), $('#within option:selected').val(), $('#vehicleType option:selected').text());
            ...
            CarsService.SearchCriteriaSave($('#make option:selected').text(), $('#model').val(), $('#yourZip').val(), $('#within option:selected').val(),$('#vehicleType option:selected').text(), OnSuccessSearchCriteriaSave, onerror);
        } else {
            hideSpinner();
...
function OnSuccessGetZips2(result, userContext, methodName) {
    try {
        ...
        if (result == true) {
            ...
            CarsSearch('All makes', $('#model').val(), zip1, '3', VehicleIDName);            
            CarsService.SearchCriteriaSave('All makes',$('#model').val(), zip1 ,'3',VehicleIDName, OnSuccessSearchCriteriaSave, onerror);
