...
var pathToImages = 'Static/Images/';	// Relative to your HTML file
...
{
	if(this.src.indexOf('over')>=0){
		this.src = pathToImages + 'close1.gif';
	}else{
		this.src = pathToImages + 'close_over1.gif';	
	}	
...
function writeTopBar()
{
	...
	var img = document.createElement('IMG');
	img.src = pathToImages + 'close1.gif';
	img.style.styleFloat = 'right';
...
function NSwriteCalendarContent()
{
	var calendarContentDivExists = true;
	if(!calendarContentDiv){
		calendarContentDiv = document.createElement('DIV');
	...
	var row = calTBody.insertRow(0);
	
	for(var no=dayArray.length-1;no>=0;no--)
	{
	...
	var cellSpaceCount=1;

	for(var no=0;no<dayStartOfMonth;no++)
	{
		var cell = row.insertCell(cellSpaceCount);
		cell.innerHTML = '&nbsp;';
		cellSpaceCount++;
	}
	...
	var rowCount=2;
	var cellCount=cellSpaceCount;	

	for(var no=1;no<=daysInMonth;no++)
	//for(var no=daysInMonth;no>0;no--)
	{
		d.setDate(no-1);
		if(colCounter>0 && colCounter%7==0){
			var row = calTBody.insertRow(rowCount);
			var cell = row.insertCell(0);
			var week = getWeek(currentYear,currentMonth,no);
			cell.innerHTML = week;		// Week	
			cell.style.backgroundColor = selectBoxRolloverBgColor;	
