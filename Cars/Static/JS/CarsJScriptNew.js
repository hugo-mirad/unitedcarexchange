...
function minMx(pMin, pMax, mMin, mMax) {
    CarsService.minMx(pMin, pMax, mMin, mMax, OnSuccessminMx, onError);
}
function OnSuccessminMx() {
    search();
}
    ...
    try {
        ...
        if (LoadingPage == 1 || LoadingPage == 3) {
            ...
            if (SearchResults != undefined) {
                ...
                $('.searchResultsHolder').append("<h2 style='color:#FF9900; text-align:center; line-height:200px; padding-bottom:150px'>No records found..!</h2>");
            }
        }
...
function EncryptedCarID(sYear, sCarID, sMake, sModel, sZip, sWithinZip,AddName) {
   ...
   var v = 'http://www.mobicarz.com/Buy-Sell-UsedCar/' + sYear + '-' + sMake + '-' + sModel.replace("&", "-") + '-' + sCarID;
...
//FindCariID for photoupload

function FindLoginCarID(sCarID) {
    //alert(sCarID);
    CarsService.FindLoginCarID(sCarID, OnSuccessFindLoginCarID, onError);
}


function OnSuccessFindLoginCarID(result, userContext, methodName) {
    try {
        // var json = $.xml2json(result);
        //console.log(result);
        productionPage(result);
    }
    catch (ex) {
    }
}

function FindRegCarID(sCarID) {
    //alert(sCarID);
    CarsService.FindRegCarID(sCarID, OnSuccessFindLoginCarID, onError);
}
