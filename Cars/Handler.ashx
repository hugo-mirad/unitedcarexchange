...
public class Handler : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        {
            System.Web.HttpPostedFile hpf = context.Request.Files[index];

            FileName = hpf.FileName.ToString();
            ...
            //}        
            HttpPostedFile uploads = context.Request.Files[FileName];
            ...
            string file2 = System.IO.Path.GetFileName(FileName);

            try
            {

                string[] Query1 = context.Request.QueryString[0].Split('*');

                string Make = Query1[0];
                if (Make == "undefined")
                    Make = context.Session["SelMake"].ToString();
                string Model = Query1[1];
                Model = Model.Replace("/", "@");
                Model = Model.Replace("&", "@");
                if (Model == "undefined") Model = context.Session["SelModel"].ToString();
                string Year = Query1[2];
                if (Year == "undefined") Year = context.Session["SelYear"].ToString();
                string CarID = Query1[3];
                if (CarID == "undefined") CarID = context.Session["CarID"].ToString();
                ...
                {
                    k = Convert.ToInt32(context.Session["k"].ToString()) + 1;
                }
                ...
                //string Filepath = "CarImages/" + Year + "/" + Make + "/" + Model + "/" + CarID + "/";
                //string FileNameFullPath = context.Server.MapPath("CarImages/" + Year + "/" + Make + "/" + Model + "/" + CarID);
                //////------------ Server in local test ----------------//
                ...
                //   string savedFileName = "C:\\Users\\padma\\Desktop\\Upload\\Upload\\CarImages\\" + FileName;

                hpf.SaveAs(FileNameSaveData);
                ...
                if (context.Session["RegUSER_ID"] != null)
                {
                    context.Session[Constants.USER_ID] = context.Session["RegUSER_ID"].ToString();
                }

                DataSet dsImgID = objdropdownBL.USP_SavePictures(Filepath, sNewFileName, "jpg", Convert.ToInt32(context.Session[Constants.USER_ID].ToString()));
                ...
                for (int j = 0; j < str.Count; j++)
                {
                    ...
                    if (str[j].ToString() != "" && str[j].ToString() != "0" && str[j].ToString() != " ")
                    {
                        ...
                        if (objcarsInfo.Pic1 == null || objcarsInfo.Pic1 == "" || objcarsInfo.Pic1 == "0")
                        {
                            ...
                            DataSet dsImgID1 = objdropdownBL.USP_SavePictures(Filepath, sNewFileName, "jpg", Convert.ToInt32(context.Session[Constants.USER_ID].ToString()));
                ...
                bool bnew = objdropdownBL.USP_UpdatePicturesById(objcarsInfo, Convert.ToInt32(context.Session[Constants.USER_ID].ToString()));
