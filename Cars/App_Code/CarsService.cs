   ...
   // ----------------------30-07-2013---------------//
    ...
    public string SubScribe(string PreferenceID, string sZip, string sName, string sEmail, string sPhoneNo)
    {
        ...
        if (dssub.Tables[0].Rows.Count == 0)
        {
            ...
            msg.Subject = "Successfully modified your personalized weekly email alerts - MobiCarz.";
            ...
            //VerifyPreferences = "www.mobicarz.com/VerifyPreferences.aspx?Preferce=" + dssub.Tables[0].Rows[0]["ID"].ToString();

            //ModifyPreferences = "www.mobicarz.com/EmailPreferences.aspx?Preferce=" + dssub.Tables[0].Rows[0]["ID"].ToString();
    ...
    [WebMethod(EnableSession = true)]
    public bool minMx(string pMin, string pMax, string mMin, string mMax)
    {
        Session["hdnPriceMIN"] = pMin.ToString();
        Session["hdnPriceMAX"] = pMax.ToString();
        Session["hdnMileageMIN"] = mMin.ToString();
        Session["hdnMileageMAX"] = mMax.ToString();
        return true;
    ...
    [ScriptMethod(UseHttpGet = false, ResponseFormat = ResponseFormat.Json)]
    [WebMethod(EnableSession = true)]
    public List<UsedCarsInfo> FindLoginCarID(string sCarid)
    {
        List<CarsInfo.UsedCarsInfo> obUsedCarsInfo = new List<CarsInfo.UsedCarsInfo>();
        UsedCarsSearch obj = new UsedCarsSearch();
        obUsedCarsInfo = (List<CarsInfo.UsedCarsInfo>)obj.FindLoginCarID(sCarid);
        return obUsedCarsInfo;
    }

    [ScriptMethod(UseHttpGet = false, ResponseFormat = ResponseFormat.Json)]
    [WebMethod(EnableSession = true)]
    public List<UsedCarsInfo> FindRegCarID(string sCarid)
    {
        List<CarsInfo.UsedCarsInfo> obUsedCarsInfo = new List<CarsInfo.UsedCarsInfo>();
        UsedCarsSearch obj = new UsedCarsSearch();
        obUsedCarsInfo = (List<CarsInfo.UsedCarsInfo>)obj.FindLoginCarIDReg(sCarid);
        return obUsedCarsInfo;
    }
